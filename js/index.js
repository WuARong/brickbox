        ...
        getBannerData(function(data){
            ...
            if(width < 768){
                ...
                for(var i = 0 ; i < datas.length ; i ++ ){
                    ...
                    html += '<a href="javascript:;" class="img_mobile hidden-sm hidden-md hidden-lg">';
                    html += '<img src="'+datas[i].img+'" alt=""/>';
                    html += '</a></div>';

                    point += '<li data-target="#banner" data-slide-to="'+i+'" class="'+(i == 0 ? "active":"")+'"></li>';
                }
            }else{
                for(var i = 0 ; i < datas.length ; i ++ ){
                    /*第一张图片才有active*/
                    html += '<div class="item '+(i == 0 ? "active":"")+'">';
                    html += '<a href="javascript:;" class="img_box hidden-xs" style="background-image: url('+datas[i].bgc+')"></a>';
                    html += '</div>';
            ...
            /*4.把html渲染在页面当中*/
            /*append() 4?*/
            $('#banner').find('.carousel-indicators').html(point);//点
            $('#banner').find('.carousel-inner').html(html);//图片
        });
    }
